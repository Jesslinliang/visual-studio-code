...
#     with write_file.write.open(mode="a", encoding="UTF-8") as write_file:
#         write_file.write("\n" + output)
...
# Function to sort items based on the first element in a tuple (used for sorting deficit_days_with_amounts)
...
def profit_function():
    ...
    for day, deficit in deficit_days_with_amounts:
        deficit_day = int(profit_loss_data[day][0])
        output += f"\n[ALL PROFIT DEFICIT] DAY: {deficit_day}, AMOUNT: USD{int(deficit)}"

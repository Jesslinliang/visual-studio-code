...
def profit_function():
    ...
    surplus_days_with_amounts.sort(key=sort_by_second_element, reverse=True)
    for day, surplus in surplus_days_with_amounts:
        surplus_day = int(profit_loss_data[day][0])
        output += f"\n[NET PROFIT SURPLUS] DAY: {surplus_day}, AMOUNT: USD{surplus}"
...
#         surplus_days_with_amounts.append((day, deficit))

#     # Sort the deficit days with amounts based on the deficit amount
#     sorted_deficit_days = sorted(surplus_days_with_amounts, key=sort_by_second_element, reverse=True)[:3]
...
#     output += "\n[TOP 3 PROFIT SURPLUS]"
#     for i, (day, surplus) in enumerate(sorted_surplus_days, start=1):
#         deficit_day = profit_loss_data[day][0]
#         output += f"\n[HIGHEST PROFIT DEFICIT ] DAY: {deficit_day}, AMOUNT: USD{deficit}"

#     # List down all the days and amounts when deficit occurs
#     output += "\n[NET PROFIT SURPLUS DAYS]"
#     surplus_days_with_amounts.sort(key=sort_by_second_element, reverse=True)
#     for day, deficit in surplus_days_with_amounts:
#         deficit_day = int(profit_loss_data[day][0])
#         output += f"\n[ALL PROFIT DEFICIT] DAY: {deficit_day}, AMOUNT: USD{deficit}"

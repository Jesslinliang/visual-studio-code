...
# Function to sort items based on the second element in a tuple (used for sorting deficit_days)
def sort_by_second_element(item):
    return item[1]

# Define the main function for processing net profit data
def net_profit_deficit_function():
    # specific absolute paths for input and output files
    file_path_read = Path(r"C:\Users\jessl\OneDrive\Microsoft Teams Chat Files\visual studio code\csv_reports\Profit_loss.csv")
    file_path_write = Path(r"C:\Users\jessl\OneDrive\Microsoft Teams Chat Files\visual studio code\summary_report.txt")
    ...
    with file_path_read.open(mode="r", encoding="UTF-8", newline="") as file:
        # Skip the header row
        header = next(file)
        net_profit_data = [list(map(float, line.strip().split(','))) for line in file]

    # Initialize an empty string to store the output message
    output = ""

    # Check if net profit is always increasing, always decreasing, or fluctuating
    surplus_increasing = all(net_profit_data[i][4] <= net_profit_data[i + 1][4] for i in range(len(net_profit_data) - 1))
    deficit_decreasing = all(net_profit_data[i][4] >= net_profit_data[i + 1][4] for i in range(len(net_profit_data) - 1))

    if surplus_increasing and not deficit_decreasing:
        output += "\n[NET PROFIT SURPLUS] Always increasing"
    elif deficit_decreasing and not surplus_increasing:
        output += "\n[NET PROFIT DEFICIT] Always decreasing"
    else:
        output += "\n[NET PROFIT] Fluctuates"
        ...
        for day in range(11, min(90, len(net_profit_data))):
            ...
            # Check if there is a net profit deficit
            if current_profit < 0:
                deficit_days_with_amounts.append((day, current_profit))

        # Sort the deficit days with amounts based on the deficit amount
        sorted_deficit_days = sorted(deficit_days_with_amounts, key=sort_by_second_element, reverse=True)
        ...
        for i, (day, deficit) in enumerate(sorted_deficit_days[:3], start=1):
            deficit_day = int(net_profit_data[day][0])
